...
class LecturerCrudController extends CrudController
{
    ...
    public function login(Request $request)
    {
        ...
        $lecturer = Lecturer::all()->where(
            'lecturer_id', $request->input("lecturerid"))->where(
    ...
    public function lectureredit($id)
    {
        ...
        return view('lectureredit')->with('lecturer',$lecturer);
    }
    public function update(Request $request , $id)
    {
        $lecturer = Lecturer::find($id);
        $lecturer->lecturer_id = $request->input('LecturerId');
        $lecturer->name = $request->input('LecturerName');
        $lecturer->save();
        session()->put('lecturerid',$request->input('LecturerId'));
        return Redirect::to("lecturer");
    ...
    public function destroy($id)
    {
        ...
        //     return Redirect::to("lecturer")->with('message', 'Unable to delete current online user');
        // }
        if(Session::get('id') == $id){
            $lecturer = Lecturer::find($id);
            $lecturer->delete();
            session()->put('lecturerid', "");
            session()->put('id', "");
            return Redirect::to("lecturer");
        }
        $lecturer = Lecturer::find($id);
